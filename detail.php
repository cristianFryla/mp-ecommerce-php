...
if ( !empty($_POST)) {
    ...
    $actualImg = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
    $actualImg .= substr($vURLImg,2, len($vURLImg)); 

    echo '<h1>' . $vTitle . '</h1>';
    echo '<h1>' . $vPrice . '</h1>';
    echo '<h1>' . $actualImg . '</h1>';
    echo '<h1>' . $vURLImg . '</h1>';
    echo '<h1>' . substr($vURLImg,2, len($vURLImg)) . '</h1>';
